(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('simpleJavaTestApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-simpleJavaTestApp-error');
                var entityKey = httpResponse.headers('X-simpleJavaTestApp-params');
                if (errorHeader) {
